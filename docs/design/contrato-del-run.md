...
**Lo que cruza entre runs.** Un documento con `origin: inherited` —el que trae una promoción, un mount o la adquisición desde un hijo— registra sus tareas con el mismo reset por identidad, y además las que el log del run fuente deja `done` nacen `done` acá, siempre que su identidad no haya cambiado: el reset gana, porque un `done` ajeno sobre una tarea que este run cortó distinto no dice nada sobre la tarea nueva. `done` es el único estado que viaja. Es el único con significado verificable en el run que recibe: `done` significa integrada, y el árbol del receptor nace del árbol donde eso está integrado. `failed` y `blocked` son lo que pedía una decisión, y la promoción o la composición *es* esa decisión, así que llegan resueltos. `running` es una tarea huérfana de un run cerrado, y una huérfana se re-corre. `ready` no se emite nunca. El log del receptor deja el cruce auditado sin evento propio: el `artifact_accepted` nombra el run fuente, y el `task_status_changed` apunta por `caused_by` al `task_registered` que lo causó.
...
Cuando el trabajo revela que el problema excede el modo (scope insuficiente, hallazgo estructural, decisión de diseño inesperada), un nodo o el engine emiten `promotion_signaled` y el run pausa en un gate. Si el humano acepta, el engine cierra el run (`run_finished: promoted`) y crea uno nuevo en un modo **posterior en el orden de declaración** (§10.1) con `promoted_from: <run_id>`, cuyo contexto inicial incluye automáticamente lo que el log del antecesor tiene en pie —sus artifacts, su documento de tareas y sus findings— (fuente `artifact` cross-run, permitida solo a través de `promoted_from`). Las tareas que el antecesor dio por hechas nacen `done` en el sucesor (§5.7), porque el árbol del sucesor nace del árbol donde ese trabajo está integrado: el sucesor sigue por donde el antecesor quedó en vez de rehacerlo. La cadena queda auditada en ambos logs. Retroceder a un modo anterior en la declaración no existe: no hay mecanismo que lo exprese.
