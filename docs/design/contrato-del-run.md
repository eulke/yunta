...
**La forma de un `kind:` se publica a quien la escribe, y un archivo ilegible se corrige.** Un `kind` declara que hay un parser detrás; declararlo sin mostrar la forma deja al que escribe adivinando una gramática que el engine tiene en tipos. Un nodo que declara `artifacts.produces: [{kind: <k>}]` recibe la forma de `<k>` como fuente de contexto derivada de esa misma declaración, en el segmento `stable` y con la ruta absoluta donde el engine la va a verificar. Fuera de un run la misma forma sale por la tool `document_shape` del plano de control y por `yunta schema <kind>`; las cuatro puertas rinden de una sola constante por kind, nombrada por un único tipo y con un test que la lee de vuelta por el mismo parser (D129, D132). **Leer un artifact interpretado es una sola puerta**: la misma lectura corre la forma y las reglas que solo valen sobre el documento entero —un id usado dos veces, una dependencia hacia una tarea que nadie declaró, dos tareas que alcanzan los mismos archivos—, así que nadie obtiene un documento sin ellas (D136). Cuando el archivo no cierra, el log registra la falla como dato y no como frase: un reporte por artifact que falló, con su path, su kind y todos sus problemas nombrados en el vocabulario del documento; el texto lo produce cada superficie al leerlo (D130, D133). El ciclo de corrección pertenece a la frontera y no a un kind de nodo: lo tiene todo nodo que resuelve un runner. La corrección es una sesión propia sobre ese runner —la forma que el nodo ya monta más los problemas, y como único trabajo reescribir los archivos declarados—, corre contra `limits.max_artifact_repairs` y cada reparación es un intento propio en el log (D131, D138). Un nodo que declara artifacts y no resuelve runner —`bash`, `check`, `gate`— no tiene ciclo, y lo dice en su tipo: no hay a quién instruir, y volver a correr un comando no es una corrección sino un reintento. Lo que una reescritura no arregla — un artifact ausente, vacío, por encima de `limits.max_artifact_bytes` o que el filesystem no entrega — falla directo, y eso lo dice el tipo de la falla, no un predicado (D134).
