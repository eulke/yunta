...
**`kind: task-ledger`** (§5) — el engine lo parsea, valida y convierte en tareas que viven en el event log. Una sesión lo entrega por `yunta_submit_task_ledger`; un nodo de comando escribe el archivo.
...
- **`yunta_request_scope_expansion`** — emite la solicitud de §6.2; solo se monta en una sesión de tarea del ledger, que es a lo que la ampliación de scope está atada.
