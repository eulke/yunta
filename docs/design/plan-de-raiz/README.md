...
| 9-03 | M32: hijos de `parallel` y nodos de fan-out abren su unidad y aterrizan; `OverlappingScope` y `OverlappingFanOutScope` pasan a ser load-bearing; el texto de `UndeclaredParallelScope` deja de prometer de más | 9-02 | bloqueado(P12) |
| 9-04 | M32: `Node.isolation`; `WorkflowIsolation` y su traductor borrados; queda `none` (D183); `inherit` deja de parsear en YAML de autor y se tolera en un manifest congelado; resume de una unidad con árbol sin aterrizar | 9-03 | pendiente |
