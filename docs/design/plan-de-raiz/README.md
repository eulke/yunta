...
| 4-02 | `ReservedIdentity`, `TemplateVar`, `ArtifactKind::Answers` (con `declarable`, `AnswersFile::against`, el montaje `kind: answers` y sus dos reglas de `check`, M26), `RecordedOrigin`, `Location`, `QuestionId`, `DiagnosticCode`, `StagedHash` | 4-01 | levantado(§11 L-45) |
...
| L-45 | 4-02 | M12 pone `Location { path: RelativePath, range: Option<LineRange> }` en `findings/mod.rs:40`, que es `FindingEntry.location` —el documento de autor—, y conectarlo cambia dos cosas que el plan no decide: `RuleCode::EmptyLocation` (`"empty-location"`, código publicado que `every_diagnostic_code_is_published` ata a `compatibility.md`) queda inalcanzable, porque un `Location` no puede estar vacío; y `From<events::Finding> for FindingEntry` deja de ser total, porque una location persistida por una versión anterior puede no parsear. Las alternativas —retirar el código publicado, o volver falible la lectura del log, o ambas— son una decisión de vocabulario visible para el usuario | el tipo se construye entero, con su `FromStr`, su `Display`, su serde y sus tests (`core/src/findings/location.rs`), y no se conecta a `FindingEntry` hasta que la decisión exista. El ítem queda levantado: lo que sí cierra en él (`RecordedOrigin`) va con su hash | fila 4-02 (§10); M12 |
