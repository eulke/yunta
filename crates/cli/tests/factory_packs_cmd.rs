...
use yunta_testkit::{git, stderr, stdout, yunta_at, Checkout};
...
/// The project a pack is installed into: the runners the factory packs
/// name, bound to the mock, and the `src/` tree a pack's own cases work
/// in.
fn setup_project() -> Checkout {
    let project = Checkout::new().config(
        "runners:\n  executor:\n    - { adapter: mock, model: mock-model }\n  reviewer:\n    - { adapter: mock, model: mock-model }\n  reviewer-alt:\n    - { adapter: mock, model: mock-model }\n",
    );
    std::fs::create_dir_all(project.repo.join("src")).unwrap();
    project.committed()
}
...
fn yunta_starter_installs_checks_and_self_tests_through_the_real_pack_pipeline() {
    ...
    let check_fix = yunta_at!(project, &["check", "yunta/fix"]);
    assert!(check_fix.status.success(), "{}", stderr(&check_fix));
    let check_review = yunta_at!(project, &["check", "yunta/review"]);
    assert!(check_review.status.success(), "{}", stderr(&check_review));

    let audit = yunta_at!(project, &["pack", "audit", "yunta/starter"]);
...
fn yunta_fragua_installs_and_checks_every_declared_mode_through_the_real_pack_pipeline() {
    ...
    git(&project.repo, &["commit", "-q", "-m", "fragua runners"]);
...
fn test_dir_runs_a_packs_own_cases_from_outside_its_root() {
    ...
    let out = yunta_at!(project, &["test", "--dir", pack.to_str().unwrap()]);
...
fn removing_a_factory_pack_leaves_the_rest_of_the_project_working() {
    let pack_source = git_ify(&repo_root().join("packs/starter"));
    let project = setup_project();
    std::fs::create_dir_all(project.repo.join(".yunta/workflows")).unwrap();
    std::fs::write(
        project.repo.join(".yunta/workflows/local.yaml"),
        "name: local\nnodes:\n  - id: noop\n    kind: bash\n    run: \"true\"\n",
    )
    .unwrap();
    ...
    let remove_out = yunta_at!(project, &["pack", "remove", "yunta/starter"]);
    assert!(remove_out.status.success(), "{}", stderr(&remove_out));
    assert!(!project.repo.join(".yunta/packs/yunta/starter").exists());

    let check = yunta_at!(project, &["check", "local"]);
    assert!(check.status.success(), "{}", stderr(&check));
}
