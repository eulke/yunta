...
#[tokio::test]
async fn a_check_leaves_on_the_log_which_layer_caught_what() {
    let bench = Bench::new();
    let plan = bench.run_dir.join("artifacts").join("plan.yaml");
    std::fs::write(
        &plan,
        "tasks:\n  - id: t1\n    title: Work\n    scope: [\"src/**\"]\n    criteria: []\n",
    )
    ...
    let (_, _) = call(&client, "yunta_check_artifact", json!({})).await;
    // The session corrects itself and asks again — the whole point of the
    // tool, and the only shape of failure a run otherwise leaves no trace of.
    std::fs::write(
        &plan,
        "tasks:\n  - id: t1\n    title: Work\n    scope: [\"src/**\"]\n    criteria:\n      - cmd: \"cargo test\"\n",
    )
    ...
    let checks: Vec<_> = events
        .iter()
        .filter_map(|e| match e.payload() {
            Some(yunta_core::events::EventPayload::ArtifactChecked(p)) => Some(p),
            _ => None,
        })
        .collect();
    assert_eq!(checks.len(), 2, "both checks are on the log");
    assert_eq!(checks[0].name, "plan.yaml");
    assert_eq!(
        checks[0].artifact_kind,
        Some(yunta_core::ArtifactKind::TaskLedger)
    );
    assert!(
        matches!(
            &checks[0].verdict,
            yunta_core::events::CheckVerdict::Problems { codes } if codes == &["no-criteria"]
        ),
        "the first check names what the session then fixed: {:?}",
        checks[0].verdict
    );
    assert_eq!(checks[1].verdict, yunta_core::events::CheckVerdict::Ok);
}
