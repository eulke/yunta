...
use yunta_core::events::{
    BaselineCapturedPayload, BaselineResults, EventPayload, FindingSeverity, HookExecutedPayload,
    HookPhase, NodeFailedPayload, NodeFinishedPayload, RunnerResolvedPayload, TokenUsage,
};
use yunta_core::{
    CheckBuiltin, HookFailurePolicy, HookStep, Hooks, JoinPolicy, Node, NodeKind, PromptSource,
};
...
pub(super) async fn execute_node(
    ...
    let end = match &node.kind {
        ...
        NodeKind::Check { builtin } => execute_check(ctx, node, builtin).await?,
    };
    Ok(end)
}
