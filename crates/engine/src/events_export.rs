...
//!
//! One JSON object per line, in `seq` order — the exact same [`StoredEvent`]
...
pub enum EventsExportError {
    ...
    Serialize { seq: Seq, detail: String },
}
...
pub fn render_events_jsonl(events: &[StoredEvent]) -> Result<String, EventsExportError> {
