...
| `yunta graph <workflow> [--run <id>] [--format mermaid\|dot]` | Renders a workflow's DAG as Mermaid (or DOT): dependencies, re-routes, parallel groups, gates — annotated with each node's derived state when `--run` names one. |
