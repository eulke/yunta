...
| `yunta pack add <source>[@ref] [--yes]` / `update <publisher>/<name> <ref>` / `remove <publisher>/<name>` / `list` | Clones, vendors and locks a third-party pack under `.yunta/packs/`, `yunta.lock` tracking exactly what's installed. Its workflows and skills are then addressable as `publisher/name` (`yunta run acme/review`, `use: acme/qa-review`, `skills: [acme/rubric]`) — see the [workflow guide](docs/guide.md#packs). A pack that ships executors needs `--yes` to confirm; `check` refuses any node that exceeds the pack's own declared permissions ceiling. |
